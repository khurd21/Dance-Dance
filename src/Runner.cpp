...
Runner::Runner() : m_currentState(GameState::Home), m_font(std::filesystem::current_path() / "assets" / "fonts" / "perfect-delight-1992.otf") {
    ...
    m_states.emplace(GameState::Play, std::make_unique<Game>(m_font, videoMode, m_eventSystem));
    m_states.emplace(GameState::SelectSong, std::make_unique<SelectSong>(m_font, videoMode, m_eventSystem));
